...
def createTrustedUser(username):
    ...
    exist_count = db.trusted.find(
        {"username": request.form["username"]}).count()

    if(exist_count > 0):
        trustedDoc = db.trusted.find_one(
            {"username": request.form["username"]})
...
def addTrustedUser(username, trusted_username):
    ...
    db.trusted.update({"username": trusted_username}, {
                      "$push": {"trusted-by-id": str(user_id)}})
    db.users.update({"username": username}, {
                    "$push": {"trustedUser": trusted_username}})
    return True
...
def TrustedUserSetPass():
    ...
    db.trusted.update({"username": request.form['username']}, {
                      "$set": {"password": passvalue}})
