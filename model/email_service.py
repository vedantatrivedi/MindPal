...
env = Environment(loader = FileSystemLoader('%s/static/email_template' % os.getcwd()))
...
def send_mail(fromaddr, toaddr, subject, message, refresh_token):
    ...
    access_token, expires_in = refresh_authorization(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, refresh_token)
    ...
    part_text = MIMEText(lxml.html.fromstring(message).text_content().encode('utf-16'), 'plain', _charset='utf-16')
