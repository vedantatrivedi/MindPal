...
def home():
    if "username" in session:
        ...
        if(check_low is True):

            last_email_date = users.get_last_email_date(session['username'])
            if(last_email_date is None or ((datetime.today() - last_email_date).days >= 15)):

                if(oauth.checkOAuthToken()):
                    token = oauth.getOAuthToken()
                    trusted_users_emails = (
                        users.getEmailofTrustedUsers(session['username']))
                    send_mail_thread = threading.Thread(target=email_service.send_welcome_mail, args=(
                        'dornumofficial@gmail.com', 'dornumofficial@gmail.com', session['username'], token['refresh_token'], request.url_root))
...
def register():
    ...
    if request.method == "GET":
        ...
        if(oauth.checkOAuthToken() and response):
            ...
            token = oauth.getOAuthToken()
            send_mail_thread = threading.Thread(target=email_service.send_welcome_mail, args=(
                'dornumofficial@gmail.com', request.form['email'], request.form['name'], token['refresh_token'], request.url_root))
            send_mail_thread.start()
...
def addPosts():
    ...
    add_post_thread = threading.Thread(
        target=users.addPost, args=(session['username'], text,))
...
def Posts():
    ...
    for entry in posts:
        ...
        allow_edits.append(
            (datetime.today()-entry[0].replace(tzinfo=None)).days < 1)
...
def trustedUser():
    ...
    if request.method == "GET":
        ...
        if(request.form["button"] == "Same User"):
            ...
            response = trusted_users.addTrustedUser(
        ...
        if(oauth.checkOAuthToken()):
            ...
            hashedUsername = trusted_users.getHashedUserName(
                request.form['username'])
            token = oauth.getOAuthToken()
            link = request.url_root + 'set_password?user=' + hashedUsername

            send_mail_thread = threading.Thread(target=email_service.send_set_pass_mail, args=(
...
def resendMail():
    ...
    if(oauth.checkOAuthToken()):
        ...
        send_mail_thread = threading.Thread(target=email_service.send_set_pass_mail, args=(
            'dornumofficial@gmail.com', request.args.get('email'), request.args.get('name'), session["username"], link, token['refresh_token']))
        send_mail_thread.start()
...
def authorize():
    ...
    #     client_config=json.loads(os.environ['CLIENT_SECRETS']), scopes=SCOPES)
...
def oauth2callback():
    ...
    state = session['state']
    # flow = google_auth_oauthlib.flow.Flow.from_client_config(
        # client_config=json.loads(os.environ['CLIENT_SECRETS']), scopes=SCOPES, state=state)
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
...
def set_password():
    ...
    if request.method == "GET":
        ...
        if(hashedUsername is None):
            return redirect(url_for("errorpage"))

        username, email = trusted_users.getUserUsingHash(hashedUsername)

        if(trusted_users.checkIfPassIsEmpty(username) is False or username is None):
            return redirect(url_for("errorpage"))
